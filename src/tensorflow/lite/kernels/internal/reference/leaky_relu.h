...
inline void QuantizeLeakyRelu<int8_t>(const LeakyReluParams& params,
                                      ...
                                      int8_t* output_data) {
  perf_enable_counter(6);
  // printf("Using custom quantized Leaky Relu\n");
  const int flat_size = MatchingFlatSize(input_shape, output_shape);
  ...
  int i = 0;
  for (; i <= flat_size - chunk_size; i += chunk_size) {
    // Send inputs
  ...
  // Handle remaining elements with software fallback
